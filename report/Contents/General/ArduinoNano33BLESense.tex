...
Der Flash-Speicher dient zur Speicherung des Programmcodes. Der SRAM wird während der Programmausführung für Variablen, Messwerte und Zwischenergebnisse verwendet. Für Anwendungen wie das Einlesen eines Sensors, das Erzeugen eines Steuersignals oder das Berechnen einfacher physikalischer Grö\ss en ist die vorhandene Rechenleistung in der Regel ausreichend.
...
Zeitmessung und Ablaufsteuerung sind eng miteinander verknüpfte Kernaufgaben von Mikrocontrollern. Eine zentrale Rolle spielt dabei die Zeitbasis des Systems, mit der der Mikrocontroller Zeitdauern erfassen und Ereignisse zu definierten Zeitpunkten auslösen kann. Statt den Programmfluss mit blockierenden Funktionen wie \PYTHON{delay()} zu unterbrechen, kann er mit der Funktion  \PYTHON{millis()} die vergangene Zeit kontinuierlich messen, Zeitabstände berechnen und anhand dieser Information entscheiden, wann der nächste Verarbeitungsschritt ausgeführt werden soll. Auf diese Weise lassen sich Sensorabfragen, Bewegungsabläufe und Berechnungen in einer zeitlich deterministischen Reihenfolge ausführen, während der Mikrocontroller gleichzeitig weitere Aufgaben bearbeitet.\cite{Hermann:2026}

Gerade in Anwendungen mit Sensoren und Aktoren ist eine solche zeitbasierte Ablaufsteuerung von besonderer Bedeutung. Wird beispielsweise ein Sensor mechanisch bewegt, wird der Zeitpunkt des Bewegungsbeginns mit \PYTHON{millis()} festgehalten und anschließend kontinuierlich überprüft, ob die geplante Wartezeit für das Einschwingen verstrichen ist. Erst wenn die Differenz zwischen der aktuellen Zeit und dem gespeicherten Startzeitpunkt grö\ss er als die gewünschte Einschwingzeit ist, wird das Messfenster geöffnet und die Sensorabfrage durchgeführt. Erfolgt die Abfrage zu früh, können die Messwerte ungenau oder schwer interpretierbar sein. Ebenso lassen sich definierte „Dead Times"' zwischen zwei Messungen realisieren, indem der Zeitpunkt der letzten Messung gespeichert und die nächste Messung erst gestartet wird, wenn ein vorgegebenes Mindestintervall überschritten ist.


Ein allgemeiner Ablauf für eine kombinierte Sensor‑Aktor‑Anwendung, bei der Zeitmessung und Ablaufsteuerung zusammenwirken, kann daher aus mehreren Schritten bestehen:

\begin{enumerate}
	\item Aktor auf eine gewünschte Position stellen und den Startzeitpunkt der Bewegung mit \PYTHON{millis()} speichern.
	\item Über die Systemzeit eine kurze Wartezeit überwachen, bis die Bewegung abgeschlossen ist, indem fortlaufend die Differenz zwischen aktueller Zeit und gespeichertem Startzeitpunkt geprüft wird, ohne den Programmablauf zu blockieren.
	\item Nach Ablauf der geplanten Wartezeit das Messfenster öffnen und die Sensorabfrage auslösen.
	\item Eingangssignal erfassen und bei Bedarf dessen Dauer oder zeitliche Lage messen, indem Start- und Endzeitpunkte des Signals über die Zeitbasis bestimmt werden.
	\item Messwert berechnen oder auswerten, beispielsweise unter Berücksichtigung von Messfenster, Dead Time und Signaldauer.
	\item Ergebnis speichern oder über eine Schnittstelle ausgeben und den Zeitpunkt dieser Ausgabe bei Bedarf wiederum zur Planung nachfolgender Aktionen heranziehen.
\end{enumerate}

Durch diese Kombination aus präziser Zeitmessung mit \PYTHON{millis()} und strukturierter Ablaufsteuerung entsteht eine klar definierte Steuerkette, in der sowohl die Reihenfolge der Schritte als auch ihre zeitlichen Abstände gezielt vorgegeben und vom Mikrocontroller zuverlässig eingehalten werden.
...
Als einfaches Beispiel für die Initialisierung der seriellen Schnittstelle und eine erste Ausgabe dient das in Listing~\ref{code:HelloWorld} gezeigte Programm. Nach dem Hochladen auf das Board und dem Öffnen des seriellen Monitors erscheint einmalig die Textausgabe „Hello World“.

 \begin{center}
 	\captionof{code}{Hello World Beispiel Code} \label{code:HelloWorld} \ArduinoExternal{}{../../Code/HelloWorld/HelloWorld/HelloWorld.ino} 
 \end{center}
...
Agus Kurniawan, IoT Projects with Arduino Nano 33 BLE Sense: Step-By-Step Projects for Beginners, Apress, 2021, ISBN: 9781484264577 \cite{Kurniawan:2021}

\begin{itemize}
	\item Dieses praxisorientierte Buch führt Schritt für Schritt in die Entwicklung mit dem Arduino Nano 33 BLE Sense ein. Es behandelt den Zugriff auf digitale und analoge I/Os, die Nutzung der integrierten Sensoren, die Bluetooth-Low-Energy-Kommunikation sowie einfache IoT- und TinyML-Anwendungen und liefert damit eine fundierte Grundlage für eigene Projekte mit diesem Board.
\end{itemize}

Claus Kühnel, Arduino – Das umfassende Handbuch, 3., aktualisierte Auflage, Rheinwerk, 2024, ISBN: 9783367102815 \cite{Kuehnel:2024}

\begin{itemize}
	\item Dieses umfassende Handbuch deckt die komplette Arduino-Plattform von den Hardwaregrundlagen über C/C++-Programmierung bis hin zu fortgeschrittenen Projekten ab. Es behandelt zahlreiche Sensoren und Aktoren, Kommunikationsschnittstellen (u.\,a. I\textsuperscript{2}C, SPI, WLAN, BLE) sowie Schaltungsdesign und liefert damit eine breite theoretische und praktische Basis für die Entwicklung eingebetteter Systeme mit Arduino, einschließlich moderner Boards wie dem Arduino Nano 33 BLE Sense.
\end{itemize}