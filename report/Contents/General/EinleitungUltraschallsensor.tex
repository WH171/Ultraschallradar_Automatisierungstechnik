...
\chapter{Stand der Technik}
	...
	Für die mechanische Ausrichtung von Sensoren und leichten Lasten werden in zahlreichen Anwendungen kompakte RC‑Servomotoren eingesetzt. Diese enthalten neben einem Gleichstrommotor ein Getriebe sowie eine integrierte Positionsregelung, die über ein Pulsweitenmodulationssignal (PWM) angesteuert wird. Miniaturservos wie der Tower Pro SG90 sind vor allem in ferngesteuerten Modellen wie Flugzeugen und Helikoptern verbreitet, wo sie etwa Ruderflächen oder andere bewegliche Komponenten präzise ansteuern.\cite{kts:2026} In technischen Lehr- und Demonstrationsaufbauten werden derartige Servos zudem häufig genutzt, um Sensoren in einem begrenzten Winkelbereich gezielt zu positionieren.
...
\chapter{Besondere Herausforderung}
	...
	Im Rahmen der Entwicklung der Applikation traten mehrere besondere Herausforderungen auf, die sowohl die mechanische Konstruktion als auch die Softwaregestaltung betrafen. Ein zentrales Ziel war der möglichst kompakte Aufbau des Systems. Dabei musste das Gehäuse so dimensioniert und gestaltet werden, dass alle Komponenten, insbesondere der Ultraschallsensor, der Servomotor und das Arduino‑Board, platzsparend integriert werden konnten, ohne die Funktionalität oder Zugänglichkeit zu beeinträchtigen. Eine zusätzliche Herausforderung bestand in der geordneten Führung und Platzierung der Anschlusskabel im Inneren des Gehäuses, um mechanische Kollisionen, Kabelbrüche sowie Beeinflussungen der Sensorik durch bewegte Leitungen zu vermeiden.
	
	Auf der Softwareseite ergaben sich weitere Anforderungen aus der gewünschten Plattformunabhängigkeit der Benutzerschnittstelle. Es sollte eine Anwendung bereitgestellt werden, die sich direkt von einem USB ‑ Stick starten lässt und möglichst ohne zusätzliche Installation auf unterschiedlichen Betriebssystemen lauffähig ist. Dies erforderte eine sorgfältige Auswahl der eingesetzten Technologien und Bibliotheken, um Abhängigkeiten zu reduzieren und eine robuste Ausführung zu ermöglichen.
	
	Schlie\ss lich zeigte sich im Verlauf der Entwicklung, dass die Visualisierung der Messdaten auf der Benutzeroberfläche zunächst zu überdimensioniert und unübersichtlich war. Einzelne Objekte wurden zu gro\ss dargestellt, sodass die eigentlichen strukturellen Informationen der Umgebung nur eingeschränkt erkennbar waren.
