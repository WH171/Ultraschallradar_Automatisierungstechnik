      ...
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          if (request.indexOf("/?CAPTURE") != -1) {
            // Funktion für den Capture-Button: Funktion Capture aufrufen
            captureText();
            client.println("<p>Bild wurde erfasst</p>");
          } else if (request.indexOf("/?TEST") != -1) {
            // Funktion für den Test-Button: LED schnell blinken lassen
            blinkLED();
          ...
          if (testExecuted) {
            ...
            testExecuted = false; // Teststatus zurücksetzen
          } else {
...
// Funktion für den Capture-Button
void captureText() {
  digitalWrite(RELAY, ON);
  delay(2000);
  digitalWrite(RELAY, OFF);
  ...
  // Hier könnte der Hexadezimal-String ausgegeben werden
